...
def update(t, lines0, lines1, images):
    ...
    for f, folder in enumerate(folders):
        ...
        for a, alpha in enumerate(alphas):
            for l, rho in enumerate(rhos):
                ...
                points = np.array([xaxis, ypc]).T.reshape(-1, 1, 2)
                ...
                lc = LineCollection(segments,
                                    ...
                                    linewidth=1,
                                    array=ypc)
                ax = lines0[f, 0, a, l].axes
                ax.add_collection(lc)

                yr = y.copy()
                points = np.array([xaxis, yr]).T.reshape(-1, 1, 2)
                segments = np.concatenate([points[:-1], points[1:]], axis=1)
                lc = LineCollection(segments,
                                    cmap=cmap,
                                    norm=norm,
    ...
    return np.concatenate([lines0.flatten(), lines1.flatten(), images.flatten()]) 
...
lines0 = np.empty_like(axlines)
lines1 = np.empty_like(axlines)
...
for f, folder in enumerate(folders):
    for c, predictor in enumerate(predictors):
        for a, alpha in enumerate(alphas):
            for l, loges in enumerate(logess):
                ax = axlines[f, c, a, l]
                lines0[f, c, a, l], = ax.plot(xaxis,
                                              dummy_y)
                lines1[f, c, a, l], = ax.plot(xaxis,
                                              dummy_y)
    for c, trait in enumerate(traits):
        ax = aximages[f, c]
...
if movie:
    ani = FuncAnimation(fig,
                        update,
                        frames=frames,
                        fargs=(lines[0], lines[1], images,),
                        blit=True)
    ani.save(filename + '.mp4', writer='ffmpeg', fps=10)
else:
    update(frame0, lines0, lines1, images,)
