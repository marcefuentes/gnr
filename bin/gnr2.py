    ...
    def chart(self, dfs, t):
        ...
        for row, (rowax, df) in enumerate(zip(axs, dfs)):
            for column, (ax, name_root) in enumerate(zip(rowax, module.c_name_roots)):
                ...
                dif = (df.loc[(df.Time == t) & (df.ChooseCost == 0.000061), name_root] - dfs[1].loc[(df.Time == t) & (df.ChooseCost == 0.000061), name_root])
                ...
                x = df.loc[(df.Time == t) & (df.ChooseCost == 0.000061), module.x_axis]
                y = df.loc[(df.Time == t) & (df.ChooseCost == 0.000061), module.y_axis]
                s = df.loc[(df.Time == t) & (df.ChooseCost == 0.000061), name_root]
