...
void write_headers_i (char *filename)
{
	...
	fprintf (fp, "alpha,logES,Given,Time,a2Default,a2Decided,a2Seen,a2SeenSum,w,ChooseGrain,MimicGrain,ImimicGrain,cost,age,chose_partner,changed_a2");
...
void write_i (char *filename, float alpha, float logES, float Given, struct itype *i, struct itype *i_last)
{
	...
	for ( ; i < i_last; i++ )
	{
		fprintf (fp, "\n%f,%f,%f,%i,%f,%f,%f,%f,%f,%f,%f,%f,%f,%i,%i,%i", alpha, logES, Given, 1, i->a2Default, i->a2Decided, i->a2Seen, i->a2SeenSum, i->wCumulative - wc, i->ChooseGrain, i->MimicGrain, i->ImimicGrain, i->cost, i->age, i->chose_partner, i->changed_a2); 
		wc = i->wCumulative;
	}
